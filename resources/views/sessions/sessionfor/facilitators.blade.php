...
<script>
    ...
    var table = $("#datatable").DataTable({
        ...
        ajax: {
            url: "{{ url('sessionForFacilitatorsData') }}/" + sessionId, // URL for fetching parent data
            data: function (d) {
                var selectedRegion = $('#regionFilter').val(); // Get the selected region from dropdown
                d.region_id = selectedRegion ? selectedRegion : regionId; // Use selected region or fallback to user's region
            }
        },
        columns: [
            { data: "DT_RowIndex", name: "DT_RowIndex", orderable: false, searchable: false }, // Index column
            { data: "name", name: "name" }, // Father Name
            { data: "contact", name: "contact" }, // Mother Name
