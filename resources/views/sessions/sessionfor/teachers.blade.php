...
<script>
    ...
    var sessionId = "{{ $sessionId }}"; // Fetch sessionId from the blade if needed
    var table = $("#datatable").DataTable({
        processing: true,
        serverSide: true,
        order: [[1, "desc"]], // Order by Teacher Name
        ajax: {
            url: "{{ url('sessionForTeachersData') }}/" + sessionId, // Correct concatenation for URL
            data: function (d) {
                d.region_id = $('#regionFilter').val(); // Pass region filter dynamically
            }
        },
        columns: [
